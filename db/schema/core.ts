...
export const comunidades = pgTable(
  'comunidades',
  {
    ...
    actualizadoEn: actualizadoEn(),
    /**
     * FR-42: `lower(unaccent(nombre))`, generated by Postgres (0055) so a search term and this
     * column are always comparable through the same `normaliza_busqueda()`. Read-only from the
     * application's side — never set on insert/update.
     */
    nombreNormalizado: text('nombre_normalizado').generatedAlwaysAs(
      sql`normaliza_busqueda(nombre)`,
    ),
  },
  (t) => [
...
export const contactos = pgTable(
  'contactos',
  {
    ...
    // FR-42: search support, generated by Postgres (0055) — read-only from the application's
    // side. `nombreNormalizado` mirrors comunidades.nombreNormalizado (same normalization, same
    // function); `telefonoDigitos` strips everything but digits so a local or E.164 search term
    // matches the same stored "+573001234567" as a substring.
    nombreNormalizado: text('nombre_normalizado').generatedAlwaysAs(sql`normaliza_busqueda(nombre)`),
    telefonoDigitos: text('telefono_digitos').generatedAlwaysAs(
      sql`regexp_replace(telefono, '\D', '', 'g')`,
    ),
  },
  (t) => [
