...
export default async function Estado() {
  ...
  if (!PUEDEN_VER.includes(sesion.rolStaff)) {
    return (
      <main>
        <h1 className="text-xl font-semibold text-barro-900">Estado</h1>
        <p className="mt-4 text-barro-700">Su rol no ve el estado del sistema.</p>
      </main>
    )
  }
  ...
  return (
    <main>
      ...
      {salud.alertas.length === 0 ? (
        <p className="mt-6 flex items-center gap-2 rounded-lg border border-selva-600 bg-selva-50 px-4 py-3 text-sm text-barro-800">
...
function Dato({
  ...
  return (
    ...
    >
      <dt className="text-sm text-barro-600">{titulo}</dt>
      <dd className="mt-0.5 font-medium text-barro-900">{valor}</dd>
      <dd className="text-sm text-barro-600">{detalle}</dd>
    </div>
  )
}
  ...
  return (
    <section className="mt-10">
      <h2 className="flex items-center gap-2 font-semibold text-barro-900">
        ...
        <span className="font-normal text-barro-600">{confirmaciones.length}</span>
      </h2>
      ...
      <ul className="mt-3 divide-y divide-barro-200 rounded-lg border border-rose-300 bg-rose-50">
        {confirmaciones.map((c) => (
          <li key={`${c.comunidad}-${c.codigo}`} className="flex flex-wrap items-baseline gap-x-2 px-4 py-3 text-sm">
            ...
            <span className="font-mono text-barro-900">{c.codigo}</span>
  ...
  return (
    ...
    <section id="silencio" className="mt-10 scroll-mt-24">
      <h2 className="flex items-center gap-2 font-semibold text-barro-900">
        ...
        <span className="font-normal text-barro-600">{comunidades.length}</span>
      </h2>
      <p className="mt-1 max-w-3xl text-sm text-barro-700">
        ...
        <ul className="mt-3 divide-y divide-barro-200 rounded-lg border border-barro-200 bg-white">
          {comunidades.map((c) => (
            <li key={c.comunidadId} className="flex flex-wrap items-baseline gap-x-2 px-4 py-3 text-sm">
              ...
              <span className="text-barro-500">tier {c.tier}</span>
              <span className="text-barro-800">
  ...
  return (
    <section className="mt-10">
      <h2 className="flex items-center gap-2 font-semibold text-barro-900">
        ...
        Daños agrupados
        <span className="font-normal text-barro-600">{agrupaciones.length}</span>
      </h2>
      <p className="mt-1 max-w-3xl text-sm text-barro-700">
        ...
        {agrupaciones.map((a, i) => (
          ...
          >
            ...
            <span className="text-barro-900">
              ...
              <span className="mt-1 block text-barro-700">
